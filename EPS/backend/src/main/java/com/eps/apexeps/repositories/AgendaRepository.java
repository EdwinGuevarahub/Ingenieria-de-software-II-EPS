...
public interface AgendaRepository extends JpaRepository<Agenda, Integer> {
    ...
    @Query("""
        ...
        JOIN t.medico m
        WHERE
            (:dniPaciente IS NULL OR a.paciente.dni = :dniPaciente)
            AND (:dniMedico IS NULL OR t.medico.dni = :dniMedico)
            AND (:dniNombrePacienteLike IS NULL OR CAST(a.paciente.dni AS string) LIKE %:dniNombrePacienteLike% OR a.paciente.nombre LIKE %:dniNombrePacienteLike%)
            AND (:dniNombreMedicoLike IS NULL OR CAST(m.dni AS string) LIKE %:dniNombreMedicoLike% OR m.nombre LIKE %:dniNombreMedicoLike%)
            AND (:cupsServicioMedico IS NULL OR s.cups = :cupsServicioMedico)
            AND (:fecha IS NULL OR DATE_TRUNC('day', a.fecha) = TO_TIMESTAMP(CAST(:fecha AS string), 'DD-MM-YYYY'))
    ...
    List<Agenda> findByPaciente_DniAndFechaBetween(
        Long pacienteId,
        Instant desde,
        Instant hasta
    );
